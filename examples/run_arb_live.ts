/**
 * A_S (Arbitrage) Strategy — Live run on Robinhood BTC market (v3, merged WS)
 *
 * ONE /stream WS connection for: market data, private fill events, and
 * signed tx submission (jsonapi/sendtx + sendtxbatch — verified live on
 * Robinhood). A full requote (cancel + cancel + bid + ask) is ONE
 * sendtxbatch message. Order operations run once per user-defined cycle.
 *
 * Run: npx tsx examples/run_arb_live.ts
 *
 * Env:
 *   LIGHTER_NETWORK=robinhood
 *   API_PRIVATE_KEY=0x...           (single key)
 *   API_PRIVATE_KEYS_JSON={"0":"0x..","1":"0x.."}   (multi-key, takes precedence)
 *   ACCOUNT_INDEX, API_KEY_INDEX
 * Optional:
 *   RUN_MINUTES=15        run duration
 *   MM_CYCLE_MS=12000     order-operations cycle (PRIMARY tuning knob)
 *   MM_ORDER_SIZE=50      order size in base units (50 = 0.0005 BTC)
 *   MM_ARB_THRESHOLD=20   half-spread $ from mark
 *   MM_REQUOTE_THRESHOLD=10  $ drift before a requote is due
 *   MM_MAX_POSITION=100   max position in base units
 *   MM_SKEW=1.0           inventory skew factor
...
const MARKET_ID = 1; // BTC perp
const ORDER_SIZE = parseInt(process.env.MM_ORDER_SIZE || '50', 10); // 0.0005 BTC
const THRESHOLD = parseInt(process.env.MM_ARB_THRESHOLD || '20', 10); // $ half-spread
const CYCLE_MS = parseInt(process.env.MM_CYCLE_MS || '12000', 10); // user-defined cycle
const REQUOTE_THRESHOLD = parseInt(process.env.MM_REQUOTE_THRESHOLD || '10', 10); // $ drift
const MAX_POSITION = parseInt(process.env.MM_MAX_POSITION || '100', 10); // 0.001 BTC
const SKEW = parseFloat(process.env.MM_SKEW || '1.0');
const MAKER_EXECUTION = true;
...
async function main() {
  ...
  console.log(`=== A_S Maker MM Strategy — Live (v3, merged WS + ${CYCLE_MS}ms cycle) ===`);
  console.log(`Network: ${network.name} | Market: ${MARKET_ID} (BTC) | Account: ${accountIndex}`);
  console.log(`API keys: ${apiPrivateKeys ? Object.keys(apiPrivateKeys).join(', ') : `${apiKeyIndex} (single)`}`);
  console.log(`Order size: ${ORDER_SIZE} units (${(ORDER_SIZE / 1e5).toFixed(5)} BTC)`);
  console.log(`Half-spread: $${THRESHOLD} | Requote drift: $${REQUOTE_THRESHOLD} | Skew: ${SKEW}`);
  console.log(`Cycle: ${CYCLE_MS}ms (~${Math.floor(60000 / CYCLE_MS)}/min) | Max position: ${MAX_POSITION} units (${(MAX_POSITION / 1e5).toFixed(5)} BTC)`);
  console.log(`Mode: ${MAKER_EXECUTION ? 'MAKER (POST_ONLY)' : 'TAKER (IOC)'} | Tx submission: /stream WS (jsonapi), HTTP fallback`);
  ...
  // 1. SignerClient (multi-key if provided)
  ...
  // 2. ONE merged WS connection: subscriptions + auth + tx submission
  ...
  // 3. Executor over the merged client (HTTP fallback rate-limited)
  const executor = new WsExecutor(signerClient, wsPrivate, {
    httpFallback: true,
    wsTxTimeoutMs: 5000,
  });
  console.log('[INIT] WsExecutor ready (WS jsonapi via /stream)');
  ...
  const strategy = new ArbitrageStrategy(
    {
      ...
      maxSlippage: 0.001,
      makerExecution: MAKER_EXECUTION,
  ...
  tracker.on('orderPlaced', (event: any) => {
    orderCount++;
    console.log(`[ORDER] #${event.order.clientOrderIndex} ${event.order.isAsk ? 'SELL' : 'BUY'} ${event.order.baseAmount} units @ ${event.order.price.toFixed(1)} status=${event.order.status}`);
  });
  ...
  executor.on('batchSent', (event: any) => {
    console.log(`[BATCH] ${event.cancels} cancels + ${event.creates} creates in 1 WS message (hashes: ${event.hashes.length})`);
  });
  ...
  const statusInterval = setInterval(() => {
    ...
    const posStr = pos && pos.size !== 0
      ...
      : ' | flat';
    console.log(`[STATUS ${elapsed}s] state=${strategy.getState()} ticks=${(strategy as any).stats.ticksProcessed} orders=${orderCount} fills=${fillCount} cancels=${cancelCount} errors=${errorCount} pendingTx=${wsPrivate.getPendingTxCount()}` +
      (md ? ` | bid=${md.bestBid.toFixed(1)} ask=${md.bestAsk.toFixed(1)} mid=${md.midPrice.toFixed(1)} mark=${md.markPrice.toFixed(1)}` : ' | no market data') + posStr);
  }, 30000);
