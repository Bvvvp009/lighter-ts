...
async function main() {
  ...
  console.log('Disabling ETH as margin (asset_index=1, ASSET_MARGIN_MODE_DISABLED=0)...');
  const [result, txHash, err] = await client.updateAccountAssetConfig(
    1,
    SignerClient.ASSET_MARGIN_MODE_DISABLED
  );
  if (err) {
    console.error('Error disabling ETH as margin:', err);
  } else {
    console.log('ETH margin disabled, tx hash:', txHash);
  }
