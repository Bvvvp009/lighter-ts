...
- **SL/TP Orders**: Grouped orders with SL/TP don't support spot markets yet. Use `createOrder` directly for spot markets.
...
npx ts-node examples/spot/create_spot_limit_order.ts
...
1. **Grouped Orders**: Grouped orders (OTOCO, OTOMA, OTOTCO) don't support spot markets yet. Use `createOrder` for individual orders.
