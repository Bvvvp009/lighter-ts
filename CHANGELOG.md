...
- **Live CLI dashboard** for all strategies: positions, orders, per-venue PnL/volume, event log, and a **Space-key config menu** for mid-run editing — scroll with arrows, Enter to edit inline (pre-filled), Enter to commit; applies next cycle and forces a requote, no restart. Handles the Windows Enter keycode (`'return'`) identically.
...
- **Config menu omitted unset knobs** (`leverage` was filtered out when undefined, so it could not be set mid-run) — now always listed.
